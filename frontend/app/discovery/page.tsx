      ...
      <main className="pt-24">
        ...
        <section className="max-w-4xl mx-auto px-6 pt-16 pb-12 text-center">
          <h1 className="text-5xl md:text-7xl font-headline font-bold mb-8 leading-tight">
            Bộ sưu tập Mẫu{" "}
            <span className="italic font-normal text-primary underline decoration-primary/20 underline-offset-8">
              Thủ công
            </span>{" "}
            Tuyển chọn
          </h1>
          <p className="text-muted-foreground max-w-2xl mx-auto mb-12 leading-relaxed text-lg">
            Khám phá các sản phẩm tinh xảo được cung cấp bởi các Người bán uy
            tín. Mỗi chi tiết đều được thiết kế sống động phục vụ nhu cầu của
            quý khách.
          </p>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="bg-muted/30 animate-pulse rounded-sm aspect-[4/5]"
                />
              ))}
            </div>
          ) : (
              ...
              {products.length > 0 ? (
                  ...
                  <div className="group lg:col-span-2">
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="text-2xl font-headline italic font-bold text-foreground">
                            {products[0].name}
                          </h3>
                          <p className="text-sm text-muted-foreground mt-2 font-medium">
                            {products[0].category?.name} •{" "}
                            {products[0].seller?.shopName}
                          </p>
                        </div>
                        <span className="shrink-0 whitespace-nowrap font-headline text-xl font-bold italic text-primary">
                          {formatCurrency(Number(products[0].price))}
                        </span>
                      </div>
                    </Link>
                  </div>
                      ...
                      <div key={product.id} className="group">
                        ...
                        >
                          ...
                          <div className="flex justify-between items-end">
                            <div>
                              <h3 className="text-xl font-headline italic font-bold text-foreground leading-tight">
                                {product.name}
                              </h3>
                            ...
                            <span className="shrink-0 whitespace-nowrap text-sm font-bold tracking-tighter text-primary">
                ...
                <div className="col-span-full py-24 text-center">
                  <p className="text-muted-foreground italic font-serif text-lg">
                    Đang cập nhật những mẫu thiết kế thủ công mới...
                  </p>
                </div>
              )}
            </div>
        ...
        <section className="max-w-7xl mx-auto px-6 mb-32">
          <div className="bg-[#F2EEE6] rounded-none p-12 md:p-24 flex flex-col md:flex-row items-center justify-between gap-16 border border-border/30 shadow-sm relative overflow-hidden">
            ...
            <div className="max-w-lg z-10">
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-primary mb-6 block">
                Dịch vụ Chế tác Riêng
              </span>
              <h2 className="text-4xl md:text-6xl font-headline font-bold mb-8 leading-tight">
                Quý khách chưa tìm thấy mẫu ưng ý?
              </h2>
              <p className="text-muted-foreground mb-10 leading-relaxed text-lg">
                Các Người bán của chúng tôi nhận sản xuất riêng theo yêu cầu.
                Hãy kết nối trực tiếp để hiện thực hóa ý tưởng của quý khách.
              </p>
